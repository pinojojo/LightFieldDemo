...
namespace ImGui
{
	...
	// 各种对齐方式的Text
	IMGUI_API void AlignedToCursorPos(const char* vText);
	IMGUI_API void AlignedToCenterText(const char* vText);

	// 对齐方式的组件
	IMGUI_API bool AlignedInputFloat(const char* vLable, float* v, float step, float step_fast, const char* format, float vStartPositionRate, float vFillRate);
	...
	IMGUI_API void ToggleButtonRect(const char* vLable, bool* v);
