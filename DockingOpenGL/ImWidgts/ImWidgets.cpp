...
bool ImGui::BeginFramedGroup(const char* vLabel, ImVec2 vSize, ImVec4 /*vCol*/, ImVec4 /*vHoveredCol*/)
{
	...
	window->ContentRegionRect.Max.x -= g.Style.IndentSpacing * 0.5;
...
void ImGui::EndFramedGroup(ImGuiCol vHoveredIdx, ImGuiCol NormalIdx)
{
	...
	const ImU32 frameCol = ImGui::GetColorU32(vHoveredIdx);
	ImGui::RenderFrame(p_min, p_max, ImGui::GetColorU32(ImGuiCol_FrameBgHovered), true, style.FrameRounding);
	ImGui::RenderFrame(p_min+ImVec2(3,25), p_max-ImVec2(3,3), ImGui::GetColorU32(NormalIdx), true, style.FrameRounding);
...
void ImGui::FramedGroupSeparator()
{
	...
	bb.Max.x = window->WorkRect.Max.x - g.Style.IndentSpacing;
...
void ImGui::AlignedToCursorPos(const char* vText)
{
	ImGui::SetCursorPosX(ImGui::GetCursorPosX()  - ImGui::CalcTextSize(vText).x);
	ImGui::Text("%s", vText);
}

void ImGui::AlignedToCenterText(const char* vText)
{
	auto windowWidth = ImGui::GetWindowSize().x;
	auto textWidth = ImGui::CalcTextSize(vText).x;

	ImGui::SetCursorPosX((windowWidth - textWidth) * 0.5f);
	ImGui::Text(vText);
}

bool ImGui::AlignedInputFloat(const char* vLable, float* v, float step, float step_fast, const char* format, float vStartPositionRate, float vFillRate)
{
	bool change = false;
	float alignedPosition = ImGui::GetWindowSize().x * vStartPositionRate;
	SetCursorPosX(alignedPosition - ImGui::GetStyle().ItemSpacing.x * 0.5);
	AlignedToCursorPos(vLable);
	SameLine();

	
	SetCursorPosX(alignedPosition + ImGui::GetStyle().ItemSpacing.x * 0.5);
	
	float widgetSizeX = ImGui::GetContentRegionAvail().x * vFillRate;
	std::string ImId = "##" + std::string(vLable);
	SetNextItemWidth(widgetSizeX);
	PushID(ImId.c_str());
	change |= InputFloat("", v, step, step_fast, format);
	PopID();
...
void ImGui::TextVertical(const char* text, ImVec2 pos, ImU32 text_color)
{
	auto drawList = ImGui::GetWindowDrawList();

	AddTextVertical(drawList, text, pos, text_color);
}
...
IMGUI_API void TextVertical(const char* text, ImVec2 pos, ImU32 text_color)
{
	
}

IMGUI_API void ImGui::ToggleButtonRect(const char* vLable, bool* v)
{
	...
	ImGui::InvisibleButton(vLable, ImVec2(width, height));
	...
	if (g.LastActiveId == g.CurrentWindow->GetID(vLable))// && g.LastActiveIdTimer < ANIM_SPEED)
	{
	...
	ImU32 col_bg;
	if (ImGui::IsItemHovered())
		col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.36f, 0.42f, 0.17f, 1.0f), ImVec4(0.27f, 0.56f, 1.0f, 1.0f), t));
	else
		col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.36f, 0.42f, 0.17f, 1.0f), ImVec4(0.27f, 0.56f, 1.0f, 1.0f), t));

	// Two Rect 
	draw_list->AddRectFilled(p, ImVec2(p.x + width, p.y + height), col_bg, g.Style.FrameRounding*2.0);
	p += ImVec2(height * 0.15, height*0.15);
	p.x += t * (width - height);
	draw_list->AddRectFilled(p, ImVec2(p.x + height*0.7, p.y + height*0.7), ImGui::GetColorU32(ImVec4(0.78,0.87,0.98,1.0)), g.Style.FrameRounding*2.2);
